# ============================================================
# Local AI DevSecOps Agent — Dockerfile
# Multi-stage build: builder installs deps, final is slim
# ============================================================

# ── Stage 1: Builder ─────────────────────────────────────────
FROM python:3.11-slim AS builder

WORKDIR /build

# Install build tools
RUN apt-get update && apt-get install -y --no-install-recommends \
    gcc \
    curl \
    unzip \
    wget \
    && rm -rf /var/lib/apt/lists/*

# Install Python dependencies into a prefix dir
COPY requirements.txt .
RUN pip install --prefix=/install --no-cache-dir \
    colorama>=0.4.6 \
    Jinja2>=3.1.2 \
    boto3>=1.34.0 \
    azure-identity>=1.15.0 \
    azure-mgmt-resource>=23.0.0 \
    google-cloud-resource-manager>=1.12.0 \
    kubernetes>=28.1.0 \
    hvac>=2.1.0 \
    pydantic>=2.0.0 \
    structlog>=23.0.0 \
    cryptography>=41.0.0

# Download Terraform binary
ARG TERRAFORM_VERSION=1.7.5
ARG TARGETARCH=amd64
RUN wget -q "https://releases.hashicorp.com/terraform/${TERRAFORM_VERSION}/terraform_${TERRAFORM_VERSION}_linux_${TARGETARCH}.zip" \
    -O /tmp/terraform.zip \
    && unzip /tmp/terraform.zip -d /tmp/terraform_bin \
    && chmod +x /tmp/terraform_bin/terraform

# Download tfsec
RUN curl -sL \
    "https://github.com/aquasecurity/tfsec/releases/latest/download/tfsec-linux-${TARGETARCH}" \
    -o /tmp/tfsec \
    && chmod +x /tmp/tfsec

# Download trivy
ARG TRIVY_VERSION=0.50.1
RUN wget -q \
    "https://github.com/aquasecurity/trivy/releases/download/v${TRIVY_VERSION}/trivy_${TRIVY_VERSION}_Linux-64bit.tar.gz" \
    -O /tmp/trivy.tar.gz \
    && tar -xzf /tmp/trivy.tar.gz -C /tmp/ \
    && chmod +x /tmp/trivy


# ── Stage 2: Final slim image ─────────────────────────────────
FROM python:3.11-slim AS final

LABEL maintainer="devsecops-agent"
LABEL description="Local AI DevSecOps Agent - Containerized"
LABEL version="1.0.0"

# Create non-root user for security
RUN groupadd -r agent && useradd -r -g agent -m -d /home/agent agent

WORKDIR /app

# Install only runtime OS dependencies
RUN apt-get update && apt-get install -y --no-install-recommends \
    git \
    ca-certificates \
    && rm -rf /var/lib/apt/lists/*

# Copy Python packages from builder
COPY --from=builder /install /usr/local

# Copy security tool binaries from builder
COPY --from=builder /tmp/terraform_bin/terraform /usr/local/bin/terraform
COPY --from=builder /tmp/tfsec                   /usr/local/bin/tfsec
COPY --from=builder /tmp/trivy                   /usr/local/bin/trivy

# Install checkov (pure Python — faster to pip install than download binary)
RUN pip install --no-cache-dir checkov

# Copy application code
COPY agent/       ./agent/
COPY main.py      ./main.py
COPY tests/       ./tests/

# Create volume mount points owned by agent user
RUN mkdir -p /app/outputs /app/logs /app/secrets /app/scan_reports \
    && chown -R agent:agent /app

# Copy entrypoint
COPY docker/entrypoint.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh

# Switch to non-root user
USER agent

# Verify tools are available
RUN terraform version && tfsec --version && echo "Tools OK"

# Volume mounts — outputs and logs persist on host
VOLUME ["/app/outputs", "/app/logs", "/app/secrets"]

# Interactive TTY needed for the terminal wizard
ENV PYTHONUNBUFFERED=1
ENV TERM=xterm-256color
ENV COLORAMA_FORCE=1

ENTRYPOINT ["/entrypoint.sh"]
CMD ["run"]
